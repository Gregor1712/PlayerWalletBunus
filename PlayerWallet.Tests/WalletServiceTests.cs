...
public class WalletServiceTests
{
    ...
    {
        var loggerFactory = LoggerFactory.Create(builder => builder.AddNLog(TestLogConfig.Create()).SetMinimumLevel(LogLevel.Debug));
        var mapper = TestMapperFactory.Create();
        var walletManager = new WalletManager(new WalletRepository(), mapper);
        var transactionManager = new TransactionManager(new TransactionRepository(), mapper);
        var lockManager = new SemaphoreSlimManager(loggerFactory.CreateLogger<SemaphoreSlimManager>());
    ...
    // Fails without using SemaphoreSlim
    [Fact]
